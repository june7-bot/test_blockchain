...
def is_valid():
    ...
    if is_valid:
        response = {'messages': 'All good it is Valid'}
    else:
        response = {'messages': 'It is Not Valid'}
    return jsonify(response), 200
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'messages': 'The node has different chains so the chain was replaced',
                    'new_chain': blockchain.chain}
    else:
        response = {'messages': 'All good the chain is the largest one',
